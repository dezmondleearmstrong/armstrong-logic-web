...
if "gemini_key" in st.secrets:
    # Explicitly using the new GenAI Client architecture
    client = genai.Client(api_key=st.secrets["gemini_key"])
    MY_EMAIL = st.secrets["my_email"]
    GMAIL_PASS = st.secrets["gmail_pass"]
    ADMIN_PASS = st.secrets.get("admin_password", "Titan2026")
else:
...
# --- 4. BRANDED SIDEBAR ---
with st.sidebar:
    st.markdown("<h1 style='color: #007bff;'>ArmstrongLogic</h1>", unsafe_content_html=True)
    ...
    if st.session_state.is_member:
        ...
        access_key = st.text_input("Enter Signature", type="password")
...
# MODE A: FULL MEMBER ACCESS (UNLIMITED GEMINI 2.5 PRO)
if st.session_state.is_member:
    ...
    if uploaded_file:
        ...
        if st.button("EXECUTE NEURAL SCAN"):
            with st.spinner("Gemini 2.5 Pro decoding profit vectors..."):
                ...
                # Target the 2.5 Pro Model explicitly
                response = client.models.generate_content(
                    model="gemini-2.5-pro", 
                    contents=f"Forensic Audit Request: Analyze this data {data_summary}. Identify 3 profit leaks. Be direct and brilliant."
                )
                st.info(response.text)
                st.balloons()

# MODE B: GUEST CALCULATOR (GEMINI 2.5 PRO LIMITED)
else:
    LIMIT = 3
    if st.session_state.leak_scans < LIMIT:
        st.markdown(f"## 🧪 Profit Leak Calculator")
        st.write(f"Cycles remaining: {LIMIT - st.session_state.leak_scans}")
        ...
        if st.button("ACTIVATE CALCULATION") and monthly_sales > 0 and target_email:
            with st.spinner(“ARMSTRONGLOGIC Engine Analyzing..."):
                try:
                    ...
                    response = client.models.generate_content(
                        model="gemini-2.5-pro", 
                        contents=f"Restaurant {res_name} does ${monthly_sales} sales. Explain 5% loss to theft/voids in 2 sentences. Mention ArmstrongLogic."
                    )
        ...
        START 7-DAY TRIAL ($99/mo)</div></a>""", unsafe_content_html=True)
